...
namespace backend.Services
{
    public class EmailService : IEmailService
    {
        ...
        public EmailService(
            ...
            IOptionsSnapshot<MailKitConfig> mailKitConfig)
        {
        ...
        public async Task SendForgotPasswordEmailAsync(ApplicationUser user, string path)
        {
            ...
            var keyValuePairs = new List<KeyValuePair<string, string>>()
            {
                ...
                new KeyValuePair<string, string>("{{url}}", $"{path}?token={passwordResetToken}&uid={user.Id}")
            };
        ...
        public async Task SendSignUpEmailAsync(ApplicationUser user, string path)
        {
            // Generate Password Reset Token -------------------------------------------------------------------------------
            string passwordResetToken = await _userManager.GeneratePasswordResetTokenAsync(user);
        ...
        private string GetHTMLBody(string htmlPage, List<KeyValuePair<string, string>> keyValuePairs)
        {
            ...
            string html = File.ReadAllText(emailTemplate);
