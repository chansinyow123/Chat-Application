...
namespace backend.Services
{
    public class FileService : IFileService
    {
        ...
        private readonly Dictionary<string, List<byte[]>> _fileSignature = new Dictionary<string, List<byte[]>>
        {
            ...
            { ".txt", new List<byte[]> { new byte[] { 0x46, 0x54, 0x58, 0x54 } } },
            { ".zip", new List<byte[]>
                {
        ...
        public async Task<byte[]> ProcessFormFileAsync(IFormFile formFile, ModelStateDictionary modelState, List<string> permittedExtensions, long fileSizeLimit, string modelName = "File" )
        {
            ...
            {
                modelState.AddModelError(modelName, "File is empty.");
            ...
            if (formFile.Length > fileSizeLimit)
            {
                ...
                modelState.AddModelError(modelName, $"File exceeds {megabyteSizeLimit:N1} MB.");
            ...
            try
            {
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    {
                        modelState.AddModelError(modelName, $"{formFile.ContentType} extension detected, File extension must be {string.Join(", ", permittedExtensions)}.");
                        return Array.Empty<byte>();
                    }
            ...
            {
                // Log the exception
                modelState.AddModelError(modelName, $"File upload failed. Error: {ex.Message}");
            }

            return Array.Empty<byte>();
        }

        private bool IsValidFileExtensionAndSignature(string fileName, Stream data, List<string> permittedExtensions)
        {
            ...
            if (!CheckFileExtension(fileName, permittedExtensions))
            {
                return false;
            }
        ...
        public bool CheckFileExtension(string fileName, List<string> permittedExtensions)
        {
            // Check the extension of the file
            var ext = Path.GetExtension(fileName).ToLowerInvariant();
